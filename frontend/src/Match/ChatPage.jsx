...
export function ChatPage(props) {
  ...
  return (
    <Draggable bounds='parent' cancel='.no-drag' nodeRef={ref}>
      <div ref={ref} className={`${styles.main} ${open ? styles.open : styles.closed}`}>
        <div className={styles.chatCtn}>
          {messages.map((msg, i) => (
            <div 
              key={i}
              className={msg.username === user.username ? styles.you : styles.other}
            >
              <div className={styles.header}>{msg.username === user.username ? 'You' : msg.username}</div>
              <div>{msg.message}</div>
            </div>
          ))}
        </div>
